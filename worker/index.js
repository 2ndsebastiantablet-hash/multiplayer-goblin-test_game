...
function createWorld(config = {}) {
  const terrain = TERRAIN_TYPES[config.terrain] ? config.terrain : 'green_hills';
  const base = TERRAIN_TYPES[terrain];
  const colors = {
    grass: cleanHex(config.colors?.grass, base.colors.grass),
    dirt: cleanHex(config.colors?.dirt, base.colors.dirt),
    stone: cleanHex(config.colors?.stone, base.colors.stone),
    water: cleanHex(config.colors?.water, base.colors.water),
    sky: cleanHex(config.colors?.sky, base.colors.sky)
  };
  return { id: crypto.randomUUID(), name: String(config.name || base.name).slice(0, 30), seed: config.seed || randSeed(), size: 65, terrain, terrainName: base.name, colors, traits: { terrain } };
}
function heightAt(x, z, world) {
  const seed = world?.seed || 1;
  const terrain = world?.terrain || world?.traits?.terrain || 'green_hills';
  let h = Math.sin((x + seed * .001) * .22) * 1.7 + Math.cos((z - seed * .001) * .24) * 1.5 + Math.sin((x + z) * .11) * 1.2 + noise(x, z, seed) * 1.4;
  if (terrain === 'desolate_wasteland') h = h * .45 - .8;
  if (terrain === 'crystal_fields') h = h * .8 + (noise(x * 2, z * 2, seed) > .89 ? 2.7 : 0);
  if (terrain === 'red_alien_plains') h = h * .7 + Math.sin(x * .55) * .5;
  if (terrain === 'jagged_highlands') h = h * 1.45 + (noise(x, z, seed) > .78 ? 2.2 : 0);
  if (terrain === 'swamp_world') h = h * .25 - .4;
  if (terrain === 'frozen_planet') h = h * .65 + (noise(x, z, seed) > .92 ? 1.5 : 0);
  if (terrain === 'ash_planet') h = h * .75 + (noise(x * 3, z * 3, seed) > .94 ? 2 : 0);
...
function canOccupy(x, z, y, world, radius = PLAYER_RADIUS) {
  const samples = [[0, 0], [radius, 0], [-radius, 0], [0, radius], [0, -radius], [radius, radius], [radius, -radius], [-radius, radius], [-radius, -radius]];
  for (const [ox, oz] of samples) if (groundY(x + ox, z + oz, world) > y + BLOCK_EPS) return false;
  return true;
}
function supportedY(x, z, world, radius = PLAYER_RADIUS) {
  const samples = [[0, 0], [radius, 0], [-radius, 0], [0, radius], [0, -radius]];
  return Math.max(...samples.map(([ox, oz]) => groundY(x + ox, z + oz, world)));
}
...
export class RoomsDO {
  ...
  async fetch(request) {
    ...
    const pair = new WebSocketPair(); const [client, server] = Object.values(pair); const clientId = crypto.randomUUID();
    server.accept(); this.clients.set(clientId, { ws: server });
    server.addEventListener('message', e => this.msg(clientId, e.data)); server.addEventListener('close', () => this.drop(clientId)); server.addEventListener('error', () => this.drop(clientId));
  ...
  msg(clientId, raw) {
    ...
    if (m.type === 'createPlanet') return this.createPlanet(clientId, m.world);
    if (m.type === 'loadPlanet') return this.loadPlanet(clientId, m.worldId);
