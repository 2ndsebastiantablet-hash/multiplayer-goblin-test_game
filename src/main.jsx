...
import VRLobby from './VRLobby.jsx';
import './styles.css';
...
function App() {
  ...
  const [showVRLobby, setShowVRLobby] = useState(false);
  const lookRef = useRef(look);
  const ws = useRef(null);
  const keys = useRef({});
  const me = useMemo(() => state.room?.players?.find(p => p.id === state.playerId), [state]);
  const isHost = !!me?.isHost;
  const optionLists = useMemo(() => ({ terrain: TERRAIN_TYPES, flora: PLANT_TRAITS, atmosphere: ATMOSPHERE_TRAITS, structure: STRUCTURE_TRAITS }), []);
  lookRef.current = look;

  function connect() { if (ws.current?.readyState === WebSocket.OPEN) return ws.current; const proto = location.protocol === 'https:' ? 'wss:' : 'ws:'; const socket = new WebSocket(`${proto}//${location.host}/ws`); ws.current = socket; socket.onmessage = ev => { const msg = JSON.parse(ev.data); if (msg.type === 'publicRooms') setRooms(msg.rooms); if (msg.type === 'joined') setState({ screen: 'game', room: msg.room, playerId: msg.playerId }); if (msg.type === 'roomUpdate') { setSavingPlanet(false); setState(s => s.room?.code === msg.room.code ? { ...s, room: msg.room } : s); } if (msg.type === 'kicked') setState({ screen: 'menu', room: null, playerId: null }); if (msg.type === 'error') { setSavingPlanet(false); alert(msg.message); } }; socket.onopen = () => socket.send(JSON.stringify({ type: 'listPublic' })); return socket; }
  ...
  function joinFirstPublic() { if (rooms[0]?.code) joinRoom(rooms[0].code); }
  ...
  useEffect(() => {
    ...
    if (!isQuestBrowser() || !xr?.isSessionSupported) return setShowVRLobby(false);
    xr.isSessionSupported('immersive-vr').then(ok => { if (!dead) setShowVRLobby(!!ok); }).catch(() => { if (!dead) setShowVRLobby(false); });
    return () => { dead = true; };
  }, []);
